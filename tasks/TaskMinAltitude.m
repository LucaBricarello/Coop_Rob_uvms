classdef TaskMinAltitude < Task   
    ...
    methods
        function updateReference(obj, robot)
            ...
            % since minimum is 1.5 we select 2 as reference
            ref_distance = 2;
        ...
        function updateActivation(obj, robot)
            if isempty(robot.altitude)
                obj.A = DecreasingBellShapedFunction(1.75, 2, 0, 1, 3);
            else
                obj.A = DecreasingBellShapedFunction(1.75, 2, 0, 1, robot.altitude);
            end
        end
    end
end